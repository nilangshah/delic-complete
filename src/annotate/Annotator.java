...
/**
 * This is the workhorse, responsible for annotating the given document. Gets a
 * document and returns an arraylist of
 * 
 * @author aman
 * 
 */
public class Annotator {
	...
	/**
	 * This function receives a document and returns a list of annotated
	 * sentences Where the annotated sentence is as defined by the class
	 * delic.AnnotatedSentence
	 * 
	 * @param licenseDoc
	 * @return
	 */
	public ArrayList<AnnotatedSentence> annotateDoc(Document licenseDoc) {
		...
		while (sentenceItr.hasNext()) { // for each sentence
			ArrayList<Concept> matchedConceptList = new ArrayList<Concept>(); // maintain
																				// a
																				// list
																				// of
																				// concepts
			...
			if (score > 2) {
				annotations.add(new AnnotatedSentence(currSentence,
						matchedConceptList, score));
			} else {
				annotations.add(new AnnotatedSentence(currSentence,
						matchedConceptList, 0));
